...
1. **SNMP Poller** queries network devices via SNMP every 60 seconds
2. **Poller** writes poll results (status, latency) to PostgreSQL
3. **Alerter** 🆕 monitors database for status changes and threshold violations
4. **Alerter** 🆕 sends notifications to PagerDuty, Slack, or email based on alert rules
5. **API Server** reads targets and poll results from PostgreSQL
6. **API Server** serves web UI and REST API endpoints (Port 8080)
7. **Web Browser** fetches data via API, displays in dashboard (auto-refresh 5s)
